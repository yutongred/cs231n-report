...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        #scores=(X.dot(W1)+b1).dot(W2)+b2       #问题：没有使用RELU！
        #1.利用np.clip
        y1=X.dot(W1)+b1
...
#         scores=y1.dot(W2)+b2
        ...
        dsoft=np.exp(scores)/np.exp(scores).sum(1,keepdims=True)
        dsoft[range(N),y]-=1
        dW2=y1.T.dot(dsoft)
        dW2/=N
        dW2+=W2*reg
    ...
    def train(self, X, y, X_val, y_val,
        ...
        for it in range(num_iters):
            ...
            indexes = np.random.choice(num_train,batch_size,replace=num_train<batch_size)#False直接要炸 用 <
            X_batch = X[indexes]
            y_batch = y[indexes]
